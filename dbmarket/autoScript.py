...
class DMarketAPI:
    ...
    def remove_minimum_price(self, title):
        ...
        with open(self.minimum_prices_file, 'r+') as file:
            ...
            if title in minimum_prices:
                ...
                file.truncate()
                self.log_action("Удалена минимальная цена ${title}")
    ...
    def set_minimum_price(self, title, price):
        try:
            ...
            self.log_action("Изменена минимальная цена ${title} на {price}")
    ...
    def add_to_ignore_list(self, title=None, offer_id=None):
        ...
        if title:
            ...
            self.log_action("Добавленно в игнорирование - {title}")
        if offer_id:
            ignore_list["offerIDs"].append(offer_id)
            self.log_action("Добавленно в игнорирование - {offer_id}")

        with open(self.ignore_list_file, 'w') as file:
            json.dump(ignore_list, file, indent=4)

    def view_ignore_list(self):
        # Проверяем, существует ли файл списка игнорирования и не пуст ли он
        if not os.path.exists(self.ignore_list_file) or os.path.getsize(self.ignore_list_file) == 0:
            print("Список игнорирования пуст.")
            return

        # Открываем файл списка игнорирования и загружаем данные
        with open(self.ignore_list_file, 'r') as file:
            ignore_list = json.load(file)

        # Проверяем, содержит ли список игнорирования записи
        if not ignore_list["titles"] and not ignore_list["offerIDs"]:
            print("Список игнорирования пуст.")
        else:
            # Выводим названия игнорируемых предметов
            if ignore_list["titles"]:
                print("Игнорируемые названия предметов:")
                for title in ignore_list["titles"]:
                    print(f"- {title}")

            # Выводим идентификаторы игнорируемых предложений
            if ignore_list["offerIDs"]:
                print("Игнорируемые идентификаторы предложений:")
                for offerID in ignore_list["offerIDs"]:
                    print(f"- {offerID}")
    ...
    def continuous_price_update(self):
        while True:
            # Здесь задаются ID игр, для которых будут обновляться цены
            game_ids = ["rust"]
            for game_id in game_ids:
                self.adjust_price_to_be_cheapest(game_id)
                print(f"Prices for game {game_id} updated.")
            time.sleep(60)  # Пауза в 1 минуту перед следующим обновлением

if __name__ == "__main__":
    dmarket_api = DMarketAPI()
    dmarket_api.continuous_price_update()
