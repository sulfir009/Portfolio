...
class ModelToolExportImport extends Model {
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		$expiry_date = $this->db->escape($product['expiry_date']);
		...
		$upc = $this->db->escape($product['barcode']);
		...
		$sql .= "`location`,`stock_status_id`, `model`,`barcode`, `expiry_date`,`manufacturer_id`,`image`,`shipping`,`price`,`points`,`date_added`,`date_modified`,`date_available`,`weight`,`weight_class_id`,`status`,";
		...
		$sql  = "INSERT INTO `".DB_PREFIX."product` (`product_id`, `quantity`, `sku`, `upc`, `model`, `manufacturer_id`, `image`, `shipping`, `price`, `points`, `date_added`, `date_modified`, `date_available`, `weight`, `weight_class_id`, `status`, `tax_class_id`, `viewed`, `stock_status_id`) VALUES ";
$sql .= "($product_id, $quantity, '$model', '$barcode', '', $manufacturer_id, '$image', $shipping, $price, $points, '$date_added', '$date_modified', '$date_available', $weight, $weight_class_id, $status, $tax_class_id, $viewed, $stock_status_id);";
		$this->db->query($sql);
		foreach ($languages as $language) {
			$language_id = $language['language_id'];
			$language_code = $language['code'];
			
			$name = isset($product['names'][$language_code]) ? $this->db->escape($product['names'][$language_code]) : '';
			$description = isset($product['descriptions'][$language_code]) ? $this->db->escape($product['descriptions'][$language_code]) : '';
			$meta_title = isset($product['meta_titles'][$language_code]) ? $this->db->escape($product['meta_titles'][$language_code]) : '';
			$meta_description = isset($product['meta_descriptions'][$language_code]) ? $this->db->escape($product['meta_descriptions'][$language_code]) : '';
			$meta_keyword = isset($product['meta_keywords'][$language_code]) ? $this->db->escape($product['meta_keywords'][$language_code]) : '';
			$tag = isset($product['tags'][$language_code]) ? $this->db->escape($product['tags'][$language_code]) : '';
	
			// Insert product description
			$sql = "INSERT INTO `" . DB_PREFIX . "product_description` (`product_id`, `language_id`, `name`, `description`, `meta_title`, `meta_description`, `meta_keyword`, `tag`, `model`, `barcode`, `expiry_date`) VALUES ";
			$sql .= "($product_id, $language_id, '$name', '$description', '$meta_title', '$meta_description', '$meta_keyword', '$tag', '$model', '$barcode', '$expiry_date');";
			$this->db->query($sql);
		}
	...
	protected function uploadProducts(&$reader, $incremental, &$available_product_ids = array()) {
		...
		for ($i = 0; $i < $k; $i += 1) {
			...
			$ean = in_array('ean', $product_fields) ? $this->getCell($data, $i, $j++, '') : '';
			$jan = in_array('jan', $product_fields) ? $this->getCell($data, $i, $j++, '') : '';
			$isbn = in_array('isbn', $product_fields) ? $this->getCell($data, $i, $j++, '') : '';
			$mpn = in_array('mpn', $product_fields) ? $this->getCell($data, $i, $j++, '') : '';
			...
			$keyword = !$this->use_table_seo_url ? $this->getCell($data, $i, $j++) : '';
			...
			$barcode = $this->getCell($data, $i, $j++, '');
			$expiry_date = $this->getCell($data, $i, $j++, '');
			...
			$product = array(
				...
				'viewed' => isset($view_counts[$product_id]) ? $view_counts[$product_id] : 0,
				'descriptions' => $descriptions,
				'stock_status_id' => $stock_status_id,
				'meta_titles' => $exist_meta_title ? $meta_titles : array(),
				...
				'mpn' => $mpn,
				'location' => $location,
				'store_ids' => ($store_ids == "") ? array() : explode(",", $store_ids),
				'related_ids' => ($related == "") ? array() : explode(",", $related),
				'layout' => ($layout == "") ? array() : explode(",", $layout),
				'subtract' => $subtract,
				'minimum' => $minimum,
				'tags' => $tags,
