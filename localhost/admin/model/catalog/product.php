...
class ModelCatalogProduct extends Model {
	...
	public function editProduct($product_id, $data) {
		...
		// За необхідності оновлення зображення продукту
		if (isset($data['image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "product SET image = '" . $this->db->escape($data['image']) . "' WHERE product_id = '" . (int)$product_id . "'");
		}
	
		// Оновлення опису продукту
...
if (!empty($data['filter_category'])) {
    $sql .= " AND p2c.category_id = '" . (int)$data['filter_category'] . "'";
		...
		// Додано фільтри за виробником, статусом наявності та штрихкодом
		if (!empty($data['filter_stock'])) {
			if ($data['filter_stock'] == '1') {
				$sql .= " AND p.quantity >= 1";
			} elseif ($data['filter_stock'] == '0') {
	...
	public function getTotalProducts($data = array()) {
		...
		if (!empty($data['filter_barcode'])) {
			$sql .= " AND p.barcode LIKE '" . $this->db->escape($data['filter_barcode']) . "%'";
		}
