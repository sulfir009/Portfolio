...
$(document).ready(function() {
	...
	$('#search input[name=\'search\']').parent().find('button').on('click', function() {
		var url = $('base').attr('href') + 'index.php?route=product/list';
		...
		$.ajax({
			...
			success: function(json) {
				...
				if (json['success']) {
					...
					setTimeout(function () {
						$('#cart > button').html('<i class="icon-bag icons"></i><span id="cart-total">' + json['total'] + '</span>');
					}, 100);
...
$(function() {
	...
	$('.manufacturer-letters  a').on('click', function() {
		$('html, body').animate({scrollTop: $(this.hash).offset().top - 230 + 'px'}, 1000, 'swing');
		return false;
	});
