...
class ModelCatalogProduct extends Model
{
	...
	{
		error_log('Getting products with model: ' . $filter_model . ' and barcode: ' . $filter_barcode);
	...
	$sql .= " AND p.barcode LIKE '" . $this->db->escape($data['filter_barcode']) . "%'";
		...
		if (!empty($data['filter_special'])) {
			if ($data['filter_special'] == 1) {
				$sql .= " AND ps.price <> 'NULL'";
			} else {
				$sql .= " AND isNull(ps.price)";
			}
		}

		if (!empty($data['filter_taste'])) {
			$sql .= " AND pf.filter_id = '" . (int)$data['filter_taste'] . "'";
		}
		error_log('SQL for getProducts: ' . $sql);
