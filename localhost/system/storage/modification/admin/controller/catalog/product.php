<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function () {
        ...
        form.addEventListener('submit', function (e) {
            ...
            // Если вам нужно все равно отправить форму, удалите строку с e.preventDefault() и добавьте form.submit();
        });
    });
</script>
...
class ControllerCatalogProduct extends Controller {
	...
	public function index() {
		...
		// Отримання списку усіх категорій
	...
	public function add() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			file_put_contents('data.log', print_r($this->request->post, true));
		}
	...
	protected function getList() {
		...
		// Збір параметрів фільтра
		$filter_keys = ['filter_stock', 'filter_manufacturer', 'filter_category', 'filter_model', 'filter_barcode', 'filter_name', 'filter_price', 'filter_quantity', 'filter_status'];
		$filter_params = [];
		foreach ($filter_keys as $key) {
			if (isset($this->request->get[$key])) {
				$filter_params[$key] = $this->request->get[$key];
		...
		$data = array_merge($data, $filter_params);
		...
		$filter_data = array_merge($filter_params, [
			'sort'  => $sort,
			'order' => $order,
		...
		foreach ($results as $result) {
			...
			$data['products'][] = [
				...
				'barcode' => $result['barcode'], // Додайте цю лінію
        'expiry_date' => $result['expiry_date'], // Додайте цю лінію
