...
async function resetImageFolder() {
  ...
  async function loadCredentials() {
    try {
        ...
        // Обратите внимание на изменение регистра ключей, чтобы они соответствовали вашему файлу JSON
        return {
...
await Promise.all([
    ...
    if (partManufacturerCodes) {
        description += `; OE: ${partManufacturerCodes}`;
            ...
            const stockElement = document.querySelector('.ordercontrol-branchcode-9 .artvalue');
let stock = 0; // Значение по умолчанию
if (stockElement) {
    const stockText = stockElement.textContent.trim(); // Получаем текстовое содержимое элемента
    const stockNumber = parseInt(stockText, 10); // Преобразуем в число
    // Изменяем логику на правильную обработку 0 и чисел больше 1
    if(stockNumber > 0) {stock = 1} else {stock = 0} 
